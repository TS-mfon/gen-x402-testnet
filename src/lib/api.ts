...
export function productHandler(product: ProductType) {
  return async (request: NextRequest) => {
    ...
    const paymentProof = request.headers.get("payment-signature") ?? request.headers.get("x-payment") ?? request.headers.get("payment");
    await appendEvent("payment.settled", { jobId: job.id, product, plan: plan.id, priceAtomic: plan.amountAtomic, clientRequestId: parsed.data.clientRequestId, paymentProof: paymentProof?.slice(0, 12000) });
