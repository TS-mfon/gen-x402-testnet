...
const schema = z.object({
  ...
  API_KEY_REGISTRY_ADDRESS: z.string().regex(/^0x[0-9a-fA-F]{40}$/).optional(),
  API_KEY_SECRET: z.string().min(32).optional(),
  API_KEY_TTL_SECONDS: z.coerce.number().int().min(60).max(365 * 24 * 60 * 60).default(30 * 24 * 60 * 60),
  API_KEY_CLOCK_SKEW_SECONDS: z.coerce.number().int().min(0).max(300).default(30),
  PLATFORM_SIGNER_PRIVATE_KEY: z.string().optional(),
...
export const env = schema.parse({
  ...
  API_KEY_REGISTRY_ADDRESS: process.env.API_KEY_REGISTRY_ADDRESS,
  API_KEY_SECRET: process.env.API_KEY_SECRET,
  API_KEY_TTL_SECONDS: process.env.API_KEY_TTL_SECONDS,
  API_KEY_CLOCK_SKEW_SECONDS: process.env.API_KEY_CLOCK_SKEW_SECONDS,
  PLATFORM_SIGNER_PRIVATE_KEY: process.env.PLATFORM_SIGNER_PRIVATE_KEY,
